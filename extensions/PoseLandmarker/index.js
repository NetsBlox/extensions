    ...
    class PoseHandle {
        ...
        async generateTask() {
            ...
            this.poseLandmarker = await Vision.Module.PoseLandmarker.createFromOptions(Vision.Task, {
                baseOptions: {
                    modelAssetPath: "http://localhost:5500/extensions/PoseLandmarker/Models/pose_landmarker_full.task",
                    delegate: "GPU"
                },
                numPoses: poseCache.options.numPoses,
                runningMode: "VIDEO",
                minPoseDetectionConfidence: poseCache.options.minPoseDetConf,
                minPosePresenceConfidence: poseCache.options.minPosePresConf,
                minTrackingConfidence: poseCache.options.minTracConf
            })
        }
        ...
        async infer(image){
            ...
            if(poseCache.data !== null && ((this.frameTime - poseCache.updateTime) < 10)){
                this.resolve = null;
                return poseCache.data;
            }
            poseCache.updateTime = this.frameTime;

            this.expiry = +new Date() + 10000;

            return new Promise(resolve => {
                poseCache.data = this.poseLandmarker.detectForVideo(image, this.frameTime);
                this.resolve = null;
                resolve(poseCache.data);
            })
        }
