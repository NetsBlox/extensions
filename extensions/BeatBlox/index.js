(function () {
    ...
    musicMorphic.onload = () => {
        window.BeatGrid.makeBeat = parent => {
            var myself = parent.ide,
                sprite = myself.currentSprite,
                dlg;

            dlg = new window.BeatGrid.BeatDialogMorph(
                null,
                beatData => {
                    if (beatData.name !== '') {
        ...
        window.BeatGrid.editBeat = parent => {
            ...
            Object.keys(vars).forEach(name => {
                if (window.BeatGrid.isBeat(vars[name])) {
                    beats.push(name);
                } 
            });
            ...
            if (beats.length === 0) {
                ...
                beats.forEach(name => {
                    menu.addItem(
                        name, 
                        () => {
                            const state = window.BeatGrid.createState(vars[name].value);

                            const dlg = new window.BeatGrid.BeatDialogMorph(
                                null,
                                beatData => {
