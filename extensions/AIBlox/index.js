...
Context.prototype.updateEmptySlots = function () {
    ...
    class AIBlox extends Extension {
        constructor(ide) {
            super('AIBlox');
            this.ide = ide;
        }
...
Answer in a LISP-like (but not specifically LISP) program representing this request where pseudocode can be used as instructions. If statements, loops, etc are valid syntax elements, but ALL other code given to them should also be pseudocode, in quotes (in parentheses where it makes sense given the syntax).

For example, 
\`("get a random ball")(if "the ball is red" ("say the ball is red then make the ball larger") ("say that's the wrong ball then bounce it away"))\`
 is a valid statement. Try to make your output like this first example.

A bad example is \`(get-ball b) (if (is-red b) (say-the-ball-is-red-and-make-larger b) (say-wrong-bounce b))\` 
This is INVALID and will be rejected as incorrect. Do not try to copy this. It is WRONG and BAD. This is not actually LISP, we don't want stupid dash names. What we want is English describing what should be done, with the structure of certain syntax.

In the \`("get a random ball")(if "the ball is red" ("say the ball is red then make the ball larger") ("say that's the wrong ball then bounce it away"))\` example, notice how the commands in the code are plain language, but they use an if syntax element along with additional parentheses to provide structure.

You do not need to define a main function or anything similar. Do not define variables. Do not include comments. Do not be vague. Do not repeat the pseudocode verbatim unless it is the only output. Your goal is to help break the request into smaller tasks.
