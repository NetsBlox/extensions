...
const DATAOPTIONS = ['Hand Landmarks', 
                     'World Landmarks',
                     'Handedness' ];

const CONFIGOPTIONS = ['Min Detect Confidence', 
...
class VisionHandler {
  ...
  async infer(image){
    ...
    if(VisionHandler.config.data !== null && ((this.frameTime - VisionHandler.config.updateTime) < 5)){
      this.resolve = null;
    ...
    return new Promise(resolve => {
      VisionHandler.config.data = this.handLandmarker.detectForVideo(image, this.frameTime);
      this.resolve = null;
