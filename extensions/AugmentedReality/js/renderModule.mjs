...
function setModelDic(){
  Object.defineProperty(models, 'box', {value: {mesh: createBoxObj(0xaa0000), scalar: 6}, writable: false});
  console.log(models);
  createDrumObj();
  createPianoObj();
}
...
function createDrumObj(){
  ...
  loader.load(modelURL, function ( gltf ) {
    ...
    Object.defineProperty(models, 'drum', {value: {mesh: drum, scalar: .1}, writable: false});
...
function createPianoObj(){
  ...
  loader.load(modelURL, function ( gltf ) {
    ...
    Object.defineProperty(models, 'piano', {value: {mesh: piano, scalar: 20}, writable: false});  
  }, undefined, function ( error ) {
    console.error( error );
  });
}
...
class ThreeJSHandler {
  ...
  createScenes(){
    ...
    this.objCamera = new three.PerspectiveCamera(40, stage.width()/stage.height(), 1, 1000); 
  ...
  updateAngles(matrix){
    ...
    [this.yaw, this.pitch, this.roll] = this.expFilter.filter(this.yaw, this.pitch, this.roll); 
  }
  ...
  createCoplanarCoords(data){
    ...
    res.push({x: Math.floor(centeredData[8].x) + sep, y: Math.floor(centeredData[8].y) - sep})
    res.push({x: Math.floor(centeredData[8].x) - sep, y: Math.floor(centeredData[8].y) - sep})
    res.push({x: Math.floor(centeredData[8].x) - sep, y: Math.floor(centeredData[8].y) + sep})
    res.push({x: Math.floor(centeredData[8].x) + sep, y: Math.floor(centeredData[8].y) + sep})
    return res;
  ...
  getSeperation(){
    ...
    const dist = Math.sqrt(((data[8].x - data[6].x))**2 + 
      ((data[8].y - data[6].y))**2 + 
