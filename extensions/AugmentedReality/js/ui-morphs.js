...
function DrumSetMorph (){
  this.init();
}

DrumSetMorph.prototype = new DialogBoxMorph();
DrumSetMorph.prototype.constructor = DrumSetMorph;
DrumSetMorph.uber = DialogBoxMorph.prototype;

DrumSetMorph.prototype.init = function () {
  DrumSetMorph.uber.init.call(this);

  this.labelString = 'Augmented Reality Instrument';
  this.createLabel();
  ...
  this.add(this.ARCode = new Morph());
  this.ARCode.setWidth(imgWidth);
  this.ARCode.setHeight(imgHeight);  
  
  const menuOptions = ['DrumSet'];

  this.add(this.IDPrompt = new StringMorph('Instrument: '));
  this.add(this.IDInput = new InputFieldMorph('0', true, menuOptions, true));
...
DrumSetMorph.prototype.download = async function () {
  
}

DrumSetMorph.prototype.updateARCode = function () {
  
}

DrumSetMorph.prototype.reactToChoice = function () {
  
}

DrumSetMorph.prototype.fixLayout = function() {
  DrumSetMorph.uber.fixLayout.call(this);
...
DrumSetMorph.prototype.accept = function () {
  if (this.action) {
    if (typeof this.target === 'function') {
      if (typeof this.action === 'function') {
        this.target.call(this.environment, this.action.call());
